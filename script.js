...
pizzaJson.map((item, index) => {
  ...
  // preencher as informações na pizzaItem
...
function mensagemFinally(){
  c('.models-finalizacao').style.display = 'flex'
  c('.models-finalizacao').style.opacity = 1
  closedModalFinish()
}

function closedModalFinish(){
  setTimeout(() => {
    c('.models-finalizacao').style.opacity = 0; // Esconde gradualmente
    setTimeout(() => {
      c('.models-finalizacao').style.display = 'none'; // Depois oculta
    }, 500); // Sincronizado com o tempo de animação
  }, 2000); //
}
